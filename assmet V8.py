...
question_db = [
    ...
    ["What percent should we still use Fossil fuels?",["Yes     -100B"],[-100, 100], [], [1,1,1,1,1]],
    ]
...
def displayChance(progress, budget):
    ...
    print("A: "+question_db[progress][1][0])
    print("B: "+question_db[progress][1][1])
    return checknuke(input(">>> "),progress, budget) 
...
def checknuke(user_answer, progress, budget):
    if user_answer.upper() == "A":
        budget += question_db[progress][2][0]
        chance = 2
        if chance == 2:
...
while progress < len(question_db):
    if len(question_db[progress][4]) == 2:
        ...
        progress, budget = displayChance(progress, budget)
        progress += 1        
